...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--line_format', choices=FORMATS.keys(),
                            default='human',
                            help='Line format (available choices: {0})'.format(
                                ', '.join(FORMATS))
                            )
    ...
    def handle(self, *args, **options):
        ...
        if line_format not in FORMATS:
            raise CommandError(
                'format must be on of {0}, not {1}'.format(line_format,
                                                           FORMATS.keys()))
        ...
        for signal in signals:
            for receiver in signal.receivers:
                ...
                print(
                    msg.format(name=name, line=inspect.getsourcelines(func)[1],
